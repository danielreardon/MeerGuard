...
while (( "$#" )); do
    ...
    if [[ $(psrstat -q -c '{$freq>2000}' ${fn}) -eq 1 ]]; then 
        echo pam -e rcvr --receiver sband_rcvr.txt --inst Asterix ${fn}
    else
        psrstat -j DTp -c all:rms -l chan=2-13,34- ${fn} | grep -v 'rms=0$' | awk '
            ...
            END {
                x=top/6.0/bot
                if (x > 5) 
                    printf("pam -e rcvr --receiver lband_rcvr.txt --inst Asterix %s\n", $1)
                else if (x < 2) 
                    printf("pam -e rcvr --receiver 7beam_rcvr.txt --inst Asterix %s\n", $1)
                else 
